...
- Let the shared review digest steer Today priority CTA and action-card emphasis. (Initial slice complete.)
...
- Keep a local archive of prepared Review packets so recap handoffs survive reloads on the same device. (Initial slice complete.)
- Add archive filters and packet export actions so saved Review packets are reusable instead of read-only. (Initial slice complete.)
